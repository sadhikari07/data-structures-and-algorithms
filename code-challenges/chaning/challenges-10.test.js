...
const count = (target, input) => {
  let newArr = [];
  var mergedArray = [].concat.apply([], input);
  console.log(mergedArray);
  newArr = mergedArray.filter(element => {
...
const totalSum = (input) => {
  ...
  newArr.push(input[i].reduce((acc, next)=> acc+next));
  }
  return newArr.reduce((acc1, next1) => acc1+next1);
  // Solution code here...
};
...
const divisibleByFiveTwoToThePower = (input) => {
    let newArr =[];
    for (let i = 0; i<input.length; i++){
    newArr.push(input[i].filter(element => {
        if (element%5===0||element===0)
        return Math.pow(2, element)
      }))
      }
      for (let i = 0; i<newArr.length; i++){
        newArr[i]=newArr[i].map(element=>Math.pow(2,element));
      }
      return newArr;
    }
...
let findMaleAndFemale = (data) => {
  ...
  newArr1 = newArr.map(element => element.name);
  return newArr1.join(' and ');
  // Solution code here...
};
...
let findShortest = (data) => {
  let newArr = [];
  newArr = data.map(element => element.name);
  console.log(newArr);
  return newArr.reduce((thisItem, nextItem) => {
    return thisItem.length<nextItem.length? thisItem:nextItem;
  })
  // Solution code here...
};
