...
let rightEmail = /^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+?\@[a-zA-Z0-9]+\.(com|org|net)$/;
return rightEmail.test(email);
  // Solution code here...
};
...
const validatePhoneNumber = (phoneNumber) => {
  let rightPhoneNumber = /^(^\(\d{3}\))|(^\d{3}[-]?)[\s]?\d{3}[\s]?[-]?\d{4}$/
  return rightPhoneNumber.test(phoneNumber);
  // Solution code here...
};
