  ...
  for (let i = 0; i<arr.length; i++){
    ...
    if (character===(arr[i].name)){
      if((arr[i].children.length)!==0){
        console.log('second if', Object.values(arr[i])[2].length);
        return true;
      }
      else return false; 
    }  
...
const totalCharacters = (arr) => {
  let arrayNames=[];
  for (let i=0; i<arr.length; i++){
if((arr[i].name!==null)&&(arr[i].spouse!==null)){
    arrayNames.push(arr[i].name);  
    arrayNames.push(arr[i].spouse);
}
Object.values(arr[i].children).forEach(value => {
  arrayNames.push(value);
});
  }
return (arrayNames.length+1);
};
