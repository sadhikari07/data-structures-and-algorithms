...
const countNumberOfElements = (arr) => {
  return arr.reduce((ansSoFar, currentValue) =>   {
   return (Math.max(arr.indexOf(currentValue), arr.indexOf(ansSoFar)))+1;
  },0);
  // // Solution code here...
}
...
const reversedString = (arr) => {
  return arr.split("").reduce((current, next) => next+current,'');
  // Solution code here...
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let childrenArr = [];
  childrenArr = arr.reduce((current, next) => {
    if('children' in next){
    for (let i=0; i<next.children.length; i++){
    current.push(next.children[i])
    }
  }
    return current;
  }, []);
...
const calculateAverage = (arr) => {
  ...
  let sumOfArray=0;
  if (arr.length>0){
    sumOfArray = arr.reduce((current, next) => current + next), 0}
